...
//在RAM定义一个Flag,之前不能再定义变量，确保此变量分配到0x20000000地址
unsigned char  Boot_value /*__attribute__ ((at(0x20000000)))*/ = 0x00;
...
#define BANK_A_SIZE_KB_MAX  (254)
#define BANK_B_OFFSET       (BANK_A_OFFSET + 1024 * BANK_A_SIZE_KB_MAX) 
#define BANK_B_SIZE_KB_MAX  (254)
...
#define APP_ENTRY_ADDRESS           (BOOT_PROGRAM_DATA_START+APP_Offset)  //app 存放起始地址
...
{volatile void (*AppEntry)(void); AppEntry = (volatile void(*)())(*(uint32_t*)(APP_ENTRY_ADDRESS+332)); AppEntry();}
